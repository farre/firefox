...
static bool IsSameDocumentPrincipal(nsIPrincipal* aCurrentlPrincipal,
                                    nsIURI* aCurrentURI, nsIURI* aNewURI) {
  ...
  nsCOMPtr<nsIURI> principalURI = aCurrentlPrincipal->GetURI();
  if (aCurrentlPrincipal->GetIsNullPrincipal()) {
    nsCOMPtr<nsIPrincipal> precursor =
        aCurrentlPrincipal->GetPrecursorPrincipal();
    if (precursor) {
...
nsresult nsDocShell::HandleSameDocumentNavigation(
  ...
  if (!IsSameDocumentPrincipal(doc->NodePrincipal(), mCurrentURI, newURI)) {
    MOZ_LOG(gSHLog, LogLevel::Debug,
    ...
    if (RefPtr<Navigation> navigation = window->Navigation()) {
      ...
      if (jsapi.Init(window)) {
        bool shouldContinue = navigation->FirePushReplaceReloadNavigateEvent(
            ...
            content->IsHTMLElement() ? content->AsElement() : nullptr,
